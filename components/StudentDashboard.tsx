  ...
  const renderHeroLayer = () => {
    ...
    return (
      <div ref={heroRef} className="flex flex-col items-center justify-start pt-6 pb-8 min-h-screen relative">
         ...
         {/* 3. PRIMARY ACTIONS (ROW 1) */}
         <div className="w-full px-4 grid grid-cols-3 gap-3 mb-3">
             {/* Mixed Quiz */}
             <button onClick={() => startAutoChallenge('DAILY')} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-2 hover:bg-slate-50 transition-colors group h-28 justify-center">
                 <div className="p-3 bg-blue-100 rounded-xl text-blue-600 group-hover:bg-blue-200 transition-colors shadow-sm">
                     <Rocket size={24} />
                 </div>
                 <p className="text-[10px] font-black text-slate-800 text-center leading-tight">Mixed Quiz</p>
             </button>

             {/* Courses */}
             <button onClick={() => onTabChange('COURSES')} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-2 hover:bg-slate-50 transition-colors group h-28 justify-center">
                 <div className="p-3 bg-orange-100 rounded-xl text-orange-600 group-hover:bg-orange-200 transition-colors shadow-sm">
                     <Book size={24} />
                 </div>
                 <p className="text-[10px] font-black text-slate-800 text-center leading-tight">Courses</p>
             </button>

             {/* Universal Video */}
             <button onClick={() => onTabChange('VIDEO')} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-2 hover:bg-slate-50 transition-colors group h-28 justify-center">
                 <div className="p-3 bg-red-100 rounded-xl text-red-600 group-hover:bg-red-200 transition-colors shadow-sm">
                     <Play size={24} fill="currentColor" />
                 </div>
                 <p className="text-[10px] font-black text-slate-800 text-center leading-tight">Video Lectures</p>
             </button>
         </div>

         {/* 4. SECONDARY ACTIONS (ROW 2) */}
         <div className="w-full px-4 grid grid-cols-3 gap-3">
             {/* Library */}
             <button onClick={() => onTabChange('COURSES')} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors">
                 <BookOpenText size={18} className="text-slate-500" />
                 <span className="text-[9px] font-bold text-slate-600">Library</span>
             </button>

             {/* Saved Notes */}
             <button onClick={() => onTabChange('PDF')} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors">
                 <FileText size={18} className="text-slate-500" />
                 <span className="text-[9px] font-bold text-slate-600">Saved Notes</span>
             </button>

             {/* History */}
             <button onClick={() => onTabChange('HISTORY')} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors">
