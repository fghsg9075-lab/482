...
const KeysTab: React.FC<KeysTabProps> = ({ providers, keys, refreshData }) => {
                             ...
                             {(providers.length === 0 || models.length === 0) && (
                                 <button
                                    onClick={() => {
                                        ...
                                        saveAIModel({id: 'gemini-1.5-flash', providerId: 'gemini', name: 'Gemini 1.5 Flash', isEnabled: true, costPerInputToken: 0, costPerOutputToken: 0});
                                        saveAIModel({id: 'gemini-1.5-pro', providerId: 'gemini', name: 'Gemini 1.5 Pro', isEnabled: true, costPerInputToken: 0, costPerOutputToken: 0});
                                        saveAIModel({id: 'llama-3.3-70b-versatile', providerId: 'groq', name: 'Llama 3.3 70B', isEnabled: true, costPerInputToken: 0, costPerOutputToken: 0});
                                        saveAIModel({id: 'llama-3.1-8b-instant', providerId: 'groq', name: 'Llama 3.1 8B', isEnabled: true, costPerInputToken: 0, costPerOutputToken: 0});
    ...
    return (
        <div className="flex flex-col h-full bg-[#0F172A] text-white p-6 rounded-2xl border border-white/10 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {activeTab === 'KEYS' && <KeysTab providers={providers} keys={keys} refreshData={refreshData} />}
